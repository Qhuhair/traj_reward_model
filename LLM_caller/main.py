...
def _extract_step(step: dict, is_standard: bool, text_only: bool = False) -> dict:
    """从单个步骤中提取所有 prompt 占位符"""
    if is_standard:
        # 优先使用标注后的图片路径（多模态模式），fallback 到原始路径
        img_before = "" if text_only else step.get("image_before_annotated") or step.get("image_before", "")
        img_after = "" if text_only else step.get("image_after_annotated") or step.get("image_after", "")
        return {
...
def run_on_trajectory(data: dict, caller: LLMCaller = None,
    ...
    step_infos = []
    for step in steps:
        info = _extract_step(step, is_std, text_only=text_only)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python main.py <input.json> [output.json] [--indices=1,3,7] [--model=xxx] [--prompt=xxx] [--text-only]")
        sys.exit(1)

    indices = _parse_indices()
    model_override = _parse_flag("model")
    prompt_override = _parse_flag("prompt")
    text_only = _has_flag("text-only")
    ...
    if existing and indices:
        new_by_step = {r["step_idx"]: r for r in results if r is not None}
        results = [
            new_by_step.get(old.get("step_idx", i + 1), old)
